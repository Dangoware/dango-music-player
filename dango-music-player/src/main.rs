...
fn main() {
    ...
    for result in query_res {
        println!("Start/End: {:?}/{:?}\nTitle: {}\nAlbum: {}\n", result.location.start().unwrap_or(&std::time::Duration::from_secs(0)), result.location.end().unwrap_or(&std::time::Duration::from_secs(0)), result.get_tag(&Tag::Title).unwrap(), result.get_tag(&Tag::Album).unwrap_or(&"".to_string()));
    }
}
